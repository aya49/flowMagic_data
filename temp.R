...
# author: alice yue
# input: 2D  csv/clr
# output: density (unnormalized) + scatterplot with no density + scatterplot with density | 600x600 ++++ plot! this is to make sure everything's ok
...
## parameters
dimsize <- 600 #_# dimension of 2D outputs
overwrite <- FALSE
...
plyr::l_ply(loop_ind, function(ii) purrr::map(ii, function(i) {
    ...
    write.table(x2discrete, file=gzfile(gs_xr(x2_file,"600/x_2Ddiscrete")),
                col.names=FALSE, row.names=FALSE, sep=",")
    ...
    write.table(dens2scat, file=gzfile(gs_xr(x2_file,"600/x_2Ddenscat")),
                col.names=FALSE, row.names=FALSE, sep=",")
    ...
    ggplot2::ggsave(paste0(gs_xr(x2_file,"600/x_2Dcontour_plot_"),".png"), gp, dpi=dimsize, height=1, width=1)
    
    # load and greyscale
    contpng <- png::readPNG(paste0(gs_xr(x2_file,"600/x_2Dcontour_plot_"),".png"))
    ...
    contpng <- 100 * contpng
    write.table(contpng, file=gzfile(gs_xr(x2_file,"600/x_2Dcontour")),
                col.names=FALSE, row.names=FALSE, sep=",")
    ...
    write.table(plotgs, file=gzfile(gs_xr(x2_file,"600/y_2D")),
                col.names=FALSE, row.names=FALSE, sep=",")
...
ggplot2::ggsave(filename=paste0(root,"/plots/2D/scores/f1_2D_baseline_600.png"), 
