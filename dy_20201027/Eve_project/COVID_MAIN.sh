...
Rscript /mnt/f/Brinkman\ group/COVID/data/code/list_files_to_analyze.R

# Identifies the number of lines in the files_to_analyze.csv, use this to create the number of sbatch arrays.
nvals=$(cat /mnt/f/Brinkman\ group/COVID/data/code/files_to_analyze.csv | wc -l) 
nvals=$(($nvals-1))

# Create the jobs in slurm
sbatch --array=1-$nvals combine_gates.sbatch
