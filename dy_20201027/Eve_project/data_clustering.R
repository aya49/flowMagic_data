...
res.dir <- "/project/COVID/data/clusters/"
...
suppressWarnings(dir.create(paste0(res.dir,ungated[g1])))
...
if (length(ind2)==1)
{
  ...
  write.table(t(c(f.name, fp.cl,fm.cl,true.clusters.n2[ind2])), 
              ...
              col.names = c("file","flowMeans","flowPeaks","truth"
                            ),sep=",",row.names = F)
}else{
write.table(t(c(f.name, fp.cl,fm.cl)), 
            file = paste0(res.dir,"/",ungated[g1],'\u20A0',f.name),
            col.names = c("file","flowMeans","flowPeaks"),sep=",",row.names = F)
}