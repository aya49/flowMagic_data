...
events_csv = pd.read_csv('/code/files_to_analyze.csv',engine='python')
print(sys.argv)
...
for u_i in user_id:
    ...
    try:
        FCM_export_temp = pd.read_csv(path, names=["filepath","score","result","minmax", "player"], header=0, sep="','", dtype={"score" :np.float64},engine='python')
        ...
        FCM_export_temp = pd.read_csv(path, names=["filepath","score","result","minmax"], header=0, sep="','", dtype={"score" :np.float64},engine='python')
        FCM_export_temp["filepath"] = FCM_export_temp["filepath"].str[1:]
        ### Now this should be applied to "player" columns
        # FCM_export_temp["minmax"] = FCM_export_temp["minmax"].str[:-1]
        FCM_export_temp["result"] = FCM_export_temp["result"].apply(json.loads)
