...
getGlobalFrame <- function(fs, sample.length=NA, all.cells=F){
  ...
  global.frame <- fsApply(fs[sample(n, sample.n)],
                          function(frame){
                            m <- nrow(frame)
                            sample.size <- ifelse (all.cells,yes = m,
                                                   no =ifelse (m<sample.n, yes = m,
                                                               no=min(m,ceiling(m/sample.n ))))
...
##############################################################################.
##############################################################################.
# Checking multimodality on axis of a bivariate plot.
get.number.of.populations <- function(obj, chans=NULL,...){
  if(class(obj)=="flowFrame"){
    if(!is.null(chans)){
      if(length(chans==2)){
        obj <- obj@exprs[,chans]
  ...
  if(ncol(obj)==2){
    densx <- density(obj[,1])
    densy <- density(obj[,2])
    peaksx <- getPeaks(densx,...)
    peaksy <- getPeaks(densy,...)
    npeaksx <- length(peaksx$Peaks)
    npeaksy <- length(peaksy$Peaks)
    peaks <- c(npeaksx,npeaksy)
    names(peaks) <- colnames(obj)
    return(peaks)
  }
}
...
##############################################################################.
# Pairs to keep
get.good.pairs <- function(f, chans=NULL){
  if(is.null(chans)){
    scat.chans <- grep("SC", colnames(f))
    time.chan <- grep("time", tolower(colnames(f)))
    marker.chans <- c(1:ncol(f))
    marker.chans <- marker.chans[-c(scat.chans,time.chan)]
  }else{
    marker.chans <- chans
  }
  peak.combination <- list()
  new.marker.chans <- marker.chans
  for(i in marker.chans){
    new.marker.chans <- new.marker.chans[-which(marker.chans==i)]
    for(j in new.marker.chans){
      peaks <- get.number.of.populations(f, chans = c(i,j))
      if(sum(peaks)>2){
        peak.combination[[length(peak.combination)+1]] <- list(markers=f@parameters@data$desc[c(i,j)], 
                                                               channels=names(peaks),
                                                               ind=c(i,j), npeaks=peaks)
      }
    }
  }
  return(peak.combination)
}
...
######################
#Func to grab markers, pairs, and all used pairs to add to the big list for the database
#This is used for Project Discovery
get.info <-function( all.fcs,
                     study.name,
                     pairs.mat=NULL,
                     output.path="~/data/Metadata/", 
                     replace.empty.markers="channels",
                     remove.pattern="-H|-W|FSC-A|FSC-H|Time|TIME|time|FS|FSC-W|SSC-B")
{
  #all.fcs is the flowSet
  #study.name is the project name, e.g., COVID_Bcell, EPIC_Myeloid,...
  #pairs.mat is the combination of pairs used to generate bivariate data
  #output.path: keep it the same if you're using Docker, otherwise save everything in that folder
  #remove.patter: This is used to remove unnecessary channels in fcs files, before calculating
  #the pairs
  f <- all.fcs[[1]]
  ...
  {
    counter <-2
    while(all(is.na(markers)) & counter <20)
    {
      f <- all.fcs[[counter]]
      markers <- as.vector(f@parameters@data[,2])
  ...
  if (all(is.na(markers)))
    stop("No markers could be found.")
  
  markers <- unlist(lapply(markers, function(x) unlist(strsplit(x," "))[1]))
  ind<-grep(markers, pattern = "Live|LIVE|VITALITY|VIABILITY|LIVE*|viability|live*",fixed = F,ignore.case = F)
  if(length(ind)==1)
    markers[ind]<-"Live"
  to.remove<-grep(chans, pattern =  remove.pattern)
  ...
  na.inds <- c(which(is.na(markers)), which(markers=="-"),
               which(markers=="NAN"),which(markers==""),which(markers==" "))
  if (length(na.inds)>0)
  {
    if (replace.empty.markers=="channels"){
       markers[na.inds]<-chans[na.inds]
    }else{
      markers[na.inds]<-replace.empty.markers
    }
  }
  #Saving marker info
  write.table(cbind(markers, chans,deparse.level = F),
              file =paste0(output.path,"/Markers_info/",study.name,"_MarkersChannels.csv" ),
              row.names = F,col.names = c("Markers","channels"),sep=",")
  #Saving file info
  file.info <- read.table("~/data/Metadata/file_info.csv",
                          check.names = F, header=T,sep = ",")
  temp.info <- data.frame(project=study.name, num_of_files=length(all.fcs))
  file.info <- rbind(file.info,temp.info,deparse.level = F)
  write.table(file.info,"~/data/Metadata/file_info.csv",
              check.names = F,row.names = F,sep = ",")
  #saving pair_info
  if (is.null(pairs.mat))
  {
    pairs.mat <- combn(chans,2)
    pairs.mat <-t(pairs.mat)
  }
  write.table( pairs.mat, file=paste0(output.path, "Project_markersComb/",study.name,"_pairs.csv",
                                    row.names = F, col.names=c("ch1","ch2"),sep=","))
  
  #Adding pairs_info to the big list
  match.markers <- apply(pairs.mat,1, function(p1) 
    paste(as.vector(markers)[match(p1, as.vector(chans))],collapse="_"))
  markers.bank <- read.table(file="~/data/Metadata/Allmarkersused.csv"
                             ,sep=",",header=T,check.names = F,fileEncoding = "UTF-7")
  for (m1 in match.markers)
  {
    ind <- match(m1, markers.bank[,1])
    if (!is.na(ind))
    {
      print("Pair exists, modifying the counts...")
    markers.bank$filecount[ind] <- markers.bank$filecount[ind]+temp.info$num_of_files
    ...
    markers.bank$projectcount[ind] <- markers.bank$projectcount[ind]+1
      ...
      print("Adding the new pair...")
      temp.bank <- data.frame(pairs=m1,filecount=length(all.fcs)
                              ,project=study.name,projectcount=1)
      markers.bank <- rbind(markers.bank,temp.bank,deparse.level = F)
      
    }
    
  }
  write.table(markers.bank,file="~/data/Metadata/Allmarkersused.csv",sep=",",row.names = F)
