...
file.info <- as.character(read.table("~/project/COVID/data/code/support_files/files_to_analyze_test_group.csv",
                        sep=",",header=T,check.names=F)[,1])
res.dir <- "~/project/COVID/Test_and_result/test_users/"
data.dir <- "~/project/COVID/data/structure_test/data/"
user.res.dir <- "~/project/COVID/data/structure_test/results/"
gold.dir <- "~/project/COVID/data/structure_test/golden_samples/"
only.col<-F
which.one <- ifelse(only.col==T,yes = "color",no="gate")
grbg <- tryCatch({lapply(file.info, function(f1){
  print(f1)
  ...
  png(paste0(res.dir,gsub(f1,pattern = "/",replacement = '\u20A0'),which.one,"_.png"),
      width = 2300,height = 700,res = 150)
  par(mfrow=c(1,4))
