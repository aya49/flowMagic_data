...
results_dir <- "/mnt/FCS_local3/backup/Brinkman group/current/Alice/impc_p2_duplicate"
...
# exception for scatterplot 09 for files: store.allFCS[i,"Barcodes"]%in%paste0("L0000555",c(80:95))
# i might have copied the wrong gates over
loop_ind <- loop_ind_f(seq_len(nrow(store.allFCS)), no_cores)
res <- plyr::llply(loop_ind, function(ii) { plyr::l_ply(ii, function(i) { try({
    if (!store.allFCS[i,"Barcodes"]%in%paste0("L0000555",c(80:95))) return()
    ...
    if (store.allFCS[i,"Barcodes"]%in%paste0("L0000555",c(80:95))) {
        gti["CD11c.gate"] <- flowDensity::deGate(temp,14)
    }
    temp <- flowDensity::flowDensity(
