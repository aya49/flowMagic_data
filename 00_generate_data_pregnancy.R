...
x_dir <- paste0(out_dir,"/nD/x/pregnancy"); dir.create(x_dir, recursive=TRUE, showWarnings=FALSE)
y_dir <- paste0(out_dir,"/nD/y/pregnancy"); dir.create(y_dir, recursive=TRUE, showWarnings=FALSE)
...
res <- furrr::future_map(loop_ind, function(ii) { purrr::map(ii, function(i) {
  ...
  try({
    ...
    csv_leuk2 <- csv_f[leukocytes.flowD@index, c("CD66","CD45"), drop=FALSE]
    clr_leuk2 <- matrix(0, nrow=nrow(csv_leuk2), ncol=3)
    colnames(clr_leuk2) <- c("granulocyte","mononuclear","other")
    clr_leuk2[id_leuk%in%granulocytes.flowD@index,1] <- 1
    clr_leuk2[id_leuk%in%mononuclear.flowD@index,2] <- 1
    clr_leuk2[rowSums(clr_leuk2)==0,3] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD66CD45_leukocyte"), showWarnings=FALSE)
      write.csv(csv_leuk2, file=gzfile(paste0(x2_dir,"/CD66CD45_leukocyte/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD66CD45_leukocyte"), showWarnings=FALSE)
      write.csv(clr_leuk2, file=gzfile(paste0(y2_dir,"/CD66CD45_leukocyte/",fid,".csv.gz")), row.names=FALSE)
      
      rm(leukocytes.flowD, leukocytes.temp, mononuclear.flowD.temp, mononuclear.temp, granulocytes, granulocytes.flowD, csv_leuk2, clr_leuk2) 
    }
    ...
    flowDensity::plotDens(
      ...
      main="*2D* CD3CD19: mononuclear", cex.lab=2, cex.axis=2, cex.main=2); 
    ...
    clr_mono <- matrix(0, nrow=nrow(csv_mono), ncol=4)
    colnames(clr_mono) <- c("Bcell","Tcell","NK lin-","other")
    clr_mono[mononuclear.flowD@index%in%Bcells.flowD@index,1] <- 1
    clr_mono[mononuclear.flowD@index%in%Tcells.flowD@index,2] <- 1
    clr_mono[mononuclear.flowD@index%in%NK.LinNeg.flowD@index,3] <- 1
    clr_mono[rowSums(clr_mono)==0,4] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD3CD19_mononuclear"), showWarnings=FALSE)
      write.csv(csv_mono, file=gzfile(paste0(x2_dir,"/CD3CD19_mononuclear/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD3CD19_mononuclear"), showWarnings=FALSE)
      write.csv(clr_mono, file=gzfile(paste0(y2_dir,"/CD3CD19_mononuclear/",fid,".csv.gz")), row.names=FALSE)
      
      rm(mononuclear.flowD, csv_mono, clr_mono) 
    }
    ...
    clr_nklin <- matrix(0, nrow=nrow(csv_nklin), ncol=2)
    colnames(clr_nklin) <- c("lin-","NK")
    clr_nklin[NK.LinNeg.flowD@index%in%LinNeg.flowD@index,1] <- 1
    clr_nklin[NK.LinNeg.flowD@index%in%NKcells.flowD@index,2] <- 1
    nklin_rows <- rowSums(clr_nklin)==1
    csv_nklin <- csv_nklin[nklin_rows,]
    clr_nklin <- clr_nklin[nklin_rows,]

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD14CD7_NKLinNeg"), showWarnings=FALSE)
      write.csv(csv_nklin, file=gzfile(paste0(x2_dir,"/CD14CD7_NKLinNeg/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD14CD7_NKLinNeg"), showWarnings=FALSE)
      write.csv(clr_nklin, file=gzfile(paste0(y2_dir,"/CD14CD7_NKLinNeg/",fid,".csv.gz")), row.names=FALSE)
      
      rm(NK.LinNeg.flowD, csv_nklin, clr_nklin) 
    }
    ...
    if (saveandrm) {
      ...
      rm(NKcells.flowD, cd56low.cd16pos.temp, cd56pos.cd16neg.flowD.temp, cd56low.cd16pos.flowD, cd56pos.cd16neg.flowD, csv_nk, clr_nk)
    }
    ...
    flowDensity::plotDens(
      ...
      main="*2D* CD14CD16: Lin-\ncMC, ncMC, & intMC", cex.lab=2, cex.axis=2, cex.main=2)
    ...
    csv_lin <- csv_f[LinNeg.flowD@index, c("CD14","CD16"), drop=FALSE]
    clr_lin <- matrix(0, nrow=nrow(csv_lin), ncol=4)
    colnames(clr_lin) <- c("cMC","ncMC","intMC","notMC")
    clr_lin[LinNeg.flowD@index%in%cMC.flowD@index,1] <- 1
    clr_lin[LinNeg.flowD@index%in%ncMC.flowD@index,2] <- 1
    clr_lin[LinNeg.flowD@index%in%intMC.flowD@index,3] <- 1
    clr_lin[LinNeg.flowD@index%in%NOT.MC.flowD@index,4] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD14CD16_lin-"), showWarnings=FALSE)
      write.csv(csv_lin, file=gzfile(paste0(x2_dir,"/CD14CD16_lin-/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD14CD16_lin-"), showWarnings=FALSE)
      write.csv(clr_lin, file=gzfile(paste0(y2_dir,"/CD14CD16_lin-/",fid,".csv.gz")), row.names=FALSE)
      
      rm(cMC.flowD, ncMC.flowD, intMC.flowD, csv_lin, clr_lin) 
    }
    ...
    #   write.csv(clr_ncMC, file=gzfile(paste0(y2_dir,"/CD123HLADR/",fid,".csv.gz")), row.names=FALSE)
      ...
      rm(ncMC.flowD, pDC.flowD, csv_ncMC, clr_ncMC)
    }
    ...
    if (saveandrm) {
      ...
      rm(ncMC.flowD, pDC.flowD, csv_nMC, clr_nMC) 
    }
    ...
    #   write.csv(clr_cMC, file=gzfile(paste0(y2_dir,"/CD123HLADR2/",fid,".csv.gz")), row.names=FALSE)
      ...
      rm(MMDSC.flowD, cMC.flowD, csv_cMC, clr_cMC) 
    }
    ...
    flowDensity::plotDens(
      ...
      main="*2D* CD4CD8: Tcell [leaf: ++]", cex.lab=2, cex.axis=2, cex.main=2,
    ...
    if (saveandrm) {
      ...
      write.csv(csv_t, file=gzfile(paste0(x2_dir,"/CD4CD8_Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD4CD8_Tcell"), showWarnings=FALSE)
      write.csv(clr_t, file=gzfile(paste0(y2_dir,"/CD4CD8_Tcell/",fid,".csv.gz")), row.names=FALSE)
      
      rm(Tcells.flowD, csv_t, clr_t) 
    }
    ...
    flowDensity::plotDens(
      ...
      main="*2D* CD4CD45RA: CD4+ Tcell", cex.lab=2, cex.axis=2, cex.main=2,
    ...
    csv_t4 <- csv_f[cd4.Tcells.flowD@index, c("CD4","CD45RA"), drop=FALSE]
    clr_t4 <- matrix(0, nrow=nrow(csv_t4), ncol=2)
    colnames(clr_t4) <- c("CD4+ naive Tcell", "CD4+ memory Tcell")
    clr_t4[cd4.Tcells.flowD@index%in%cd4.T.Naive.flowD@index,1] <- 1
    clr_t4[cd4.Tcells.flowD@index%in%cd4.T.Memory.flowD@index,2] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD4CD45RA_Tcell"), showWarnings=FALSE)
      write.csv(csv_t4, file=gzfile(paste0(x2_dir,"/CD4CD45RA_Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD4CD45RA_Tcell"), showWarnings=FALSE)
      write.csv(clr_t4, file=gzfile(paste0(y2_dir,"/CD4CD45RA_Tcell/",fid,".csv.gz")), row.names=FALSE)
      
      rm(cd4.T.Memory.flowD, csv_t4, clr_t4) 
    }
    ...
    if (saveandrm) {
      ...
      rm(cd4.Tcells.flowD, csv_t4Th, clr_t4Th) 
    }
    ...
    csv_cd4Tn <- csv_f[cd4.T.Naive.flowD@index, c("FoxP3","CD25"), drop=FALSE]
    clr_cd4Tn <- matrix(0, nrow=nrow(csv_cd4Tn), ncol=2)
    colnames(clr_cd4Tn) <- c("Tregs naive Tcell","CD4+ naive Tcell other")
    clr_cd4Tn[cd4.T.Naive.flowD@index%in%Tregs.Naive.flowD@index,1] <- 1
    clr_cd4Tn[clr_cd4Tn[,1]==0,2] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/FoxP3CD25_CD4Tcell"), showWarnings=FALSE)
      write.csv(csv_cd4Tn, file=gzfile(paste0(x2_dir,"/FoxP3CD25_CD4Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/FoxP3CD25_CD4Tcell"), showWarnings=FALSE)
      write.csv(clr_cd4Tn, file=gzfile(paste0(y2_dir,"/FoxP3CD25_CD4Tcell/",fid,".csv.gz")), row.names=FALSE)
      
      rm(cd4.T.Naive.flowD, Tregs.Naive.flowD, csv_cd4Tn, clr_cd4Tn) 
    }
    ...
    csv_ncd4cd8T <- csv_f[NOT.cd4.cd8.Tcells.flowD@index, c("TCRgd","CD3"), drop=FALSE]
    clr_ncd4cd8T <- matrix(0, nrow=nrow(csv_ncd4cd8T), ncol=2)
    colnames(clr_ncd4cd8T) <- c("gamma-delta Tcell","CD4-CD8- Tcell other")
    clr_ncd4cd8T[NOT.cd4.cd8.Tcells.flowD@index%in%gammaDelta.Tcells.flowD@index,1] <- 1
    clr_ncd4cd8T[clr_ncd4cd8T[,1]==0,2] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/TCRgdCD3_CD4Tcell"), showWarnings=FALSE)
      write.csv(csv_ncd4cd8T, file=gzfile(paste0(x2_dir,"/TCRgdCD3_CD4Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/TCRgdCD3_CD4Tcell"), showWarnings=FALSE)
      write.csv(clr_ncd4cd8T, file=gzfile(paste0(y2_dir,"/TCRgdCD3_CD4Tcell/",fid,".csv.gz")), row.names=FALSE)
      
      rm(NOT.cd4.cd8.Tcells.flowD, gammaDelta.Tcells.flowD, csv_ncd4cd8T, clr_ncd4cd8T) 
    }
    ...
    csv_Tcd8 <- csv_f[cd8.Tcells.flowD@index, c("CD8","CD45RA"), drop=FALSE]
    clr_Tcd8 <- matrix(0, nrow=nrow(csv_Tcd8), ncol=2)
    colnames(clr_Tcd8) <- c("CD8+ naive Tcell","CD8+ memory Tcell")
    clr_Tcd8[cd8.Tcells.flowD@index%in%cd8.T.Naive.flowD@index,1] <- 1
    clr_Tcd8[cd8.Tcells.flowD@index%in%cd8.T.Memory.flowD@index,2] <- 1

    if (saveandrm) {
      dir.create(paste0(x2_dir,"/CD8CD45RA_notCD4CD8Tcell"), showWarnings=FALSE)
      write.csv(csv_Tcd8, file=gzfile(paste0(x2_dir,"/CD8CD45RA_notCD4CD8Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/CD8CD45RA_notCD4CD8Tcell"), showWarnings=FALSE)
      write.csv(clr_Tcd8, file=gzfile(paste0(y2_dir,"/CD8CD45RA_notCD4CD8Tcell/",fid,".csv.gz")), row.names=FALSE)
      
      rm(cd8.T.Naive.flowD, cd8.T.Memory.flowD, csv_Tcd8, clr_Tcd8) 
    }
    ...
    flowDensity::plotDens(
      ...
      main="CD25+CD8+ T Naive & CD25+CD8+ T Memory", cex.lab=2, cex.axis=2, cex.main=2)
    ...
    csv_Tcd82 <- csv_f[cd8.Tcells.flowD@index, c("Tbet","CD45RA"), drop=FALSE]
    clr_Tcd82 <- matrix(0, nrow=nrow(csv_Tcd82), ncol=3)
    colnames(clr_Tcd82) <- c("CD25+CD8+ naive Tcell","CD25+CD8+ memory Tcell", "CD25+CD8+ Tcell other")
    clr_Tcd82[cd8.Tcells.flowD@index%in%cd25.cd8.T.Naive.flowD@index,1] <- 1
    clr_Tcd82[cd8.Tcells.flowD@index%in%cd25.cd8.T.Memory.flowD@index,2] <- 1
    clr_Tcd82[rowSums(clr_Tcd82)==0,3] <- 1
    
    if (saveandrm) {
      dir.create(paste0(x2_dir,"/TbetCD45RA2_CD8Tcell"), showWarnings=FALSE)
      write.csv(csv_Tcd82, file=gzfile(paste0(x2_dir,"/TbetCD45RA2_CD8Tcell/",fid,".csv.gz")), row.names=FALSE)
      dir.create(paste0(y2_dir,"/TbetCD45RA2_CD8Tcell"), showWarnings=FALSE)
      write.csv(clr_Tcd82, file=gzfile(paste0(y2_dir,"/TbetCD45RA2_CD8Tcell/",fid,".csv.gz")), row.names=FALSE)
