...
no_cores <- 3#parallel::detectCores() - 5
...
gs_xr_train <- function(x) gs_xr(x,"x_2Ddenscat_euclidean_rankkmed","data") 
...
overwrite_plot <- TRUE
plot_train_only <- FALSE

israndom <- FALSE
flowLearn_supp <- FALSE

start <- Sys.time()

# li <- loop_ind_f(thres_dirs, no_cores)
# res <- plyr::l_ply(li, function(thres_dirs_) { #try({
...
for (thres_dir_ in thres_dirs) { try({
    ...
    ## TEMP
    train_dir <- gs_xr_train(thres_dir_)
    ks <- sort(as.numeric(list.dirs(train_dir, full.names=FALSE)))
    # if (overwrite_thresholds) {
    protoIdxs_fnames <- NULL # for if we want flowlearn to choose train samples
    if (!flowLearn_supp)
        protoIdxs_fnames <- purrr::map(paste0(train_dir,"/",ks), function(x) {
            trains <- list.files(x, full.names=FALSE)
            if (israndom)
                return(trains)
            z <- lapply(trains, function(y) 
                data.table::fread(paste0(x,"/",y), data.table=FALSE)[,1])
            names(z) <- gsub(".csv.gz","",trains)
            return(z)
        })
    for (markname in marknames) {
        dt_dir <- paste0(fl_dir,"/",markname)
        # if (!overwrite_thresholds & all(sapply(paste0(dt_dir,"/",ks,".Rdata"), file.exists))) next
        
        # make a density data object for flowLearn
        densdat <- new('DensityData')
        fnames_ <- fnames[sapply(fnames, function(fname) !is.na(ftos[[fname]][markname]))]
        ...
        if (flowLearn_supp)#(is.null(protoIdxs_fnames))
            protoIdxs_fnames <- plyr::llply(ks, function(k)
                which(rownames(densdat@data)%in%flowLearn::flSelectPrototypes(densdat, k)))
        
        dir.create(dt_dir, showWarnings=FALSE, recursive=TRUE)
        protoIdxs <- lapply(seq_len(length(ks)), function(ki) {
            if (flowLearn_supp) return(protoIdxs_fnames[[ki]])
            if (israndom) return(match(protoIdxs_fnames[[ki]], fnames_))
            match(names(protoIdxs_fnames[[ki]]), fnames_) })
    ...
    for (k in ks_) {
        fly_dir <- paste0(gsub("thresholds","labels",fl_dir),"/",k)
        ...
        for (fname in fnames) {
            ...
            tpm_ <- matrix(0, nrow=nrow(tpm), ncol=ncol(tpm))
            tpm_[which(tpm, arr.ind=TRUE)] <- 1
            write.table(tpm_, file=gzfile(paste0(fly_dir,"/",fname,".csv.gz")), 
    ...
    for (fname in fnames) {
        for (k in ks_) {
            trainf <- any(grepl(fname, fnames_[protoIdxs[[which(ks==k)]]]))
            if (plot_train_only & !trainf) next
            png_name <- paste0(pl_dir,"/",k,"/",fname,ifelse(trainf,"_train",""),".png")
    ...
    rm(testpars,ftos, x2s, y2s, filt2s); gc()
