...
gs_xr_ <- function(x,y="Rtsne") gs_xr(x,y)
plyr::l_ply(gs_xr_(xn_dirs), dir.create, recursive=TRUE, showWarnings=FALSE)
...
xn_files <- xn_files[sapply(xn_files, function(x) {
  if (file.exists(gs_xr_(x))) if (file.size(gs_xr_(x))>0) return(FALSE)
  return(TRUE)
})]
...
for (i in fe) {
  ...
  write.table(tx, file=gzfile(gs_xr_(xn_f)), sep=",", row.names=FALSE, col.names=TRUE)
